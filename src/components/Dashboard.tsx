...
import { Plus, Search, Filter, CreditCard as Edit, Trash2, Calendar, Wheat, Package, QrCode, ScanLine, Truck, Store, User, ShoppingCart, Brain, Settings } from 'lucide-react';
...
const Dashboard: React.FC = () => {
  ...
  const loadCrops = () => {
    ...
    // Load crops from API
    apiService.getCrops().then(result => {
      if (result.data) {
  ...
  const handleAddCrop = (cropData: Omit<Crop, 'id' | 'user_id' | 'created_at'>) => {
    ...
    apiService.createCrop(cropData).then(result => {
      if (result.data) {
        loadCrops();
      }
    });
  ...
  const handleUpdateCrop = (cropId: string, cropData: Omit<Crop, 'id' | 'user_id' | 'created_at'>) => {
    apiService.updateCrop(cropId, cropData).then(result => {
      if (result.data) {
        loadCrops();
      }
    });
  ...
  const handleScanResult = (cropId: string) => {
    apiService.getCropForScanning(cropId).then(result => {
      if (result.data) {
  ...
  return (
    <div className="flex gap-6">
      {/* Left Sidebar - Weather and Price Trends */}
      ...
      <div className="flex-1 space-y-6">
        ...
        {filteredCrops.length === 0 ? (
          <div className="bg-white rounded-xl shadow-lg border border-orange-200 p-12 text-center">
            ...
            {!searchTerm && filterType === 'all' && user?.role === 'farmer' && (
              <button
                onClick={() => setShowForm(true)}
